...
import { keywordInItem } from './utilities/keywordactions.js';
import { RequestOptions, HttpMethod } from '../../http/requestoptions.js';
import { RunRequest } from '../../http/httprequest.js';
import { AddKeyword, UpdateKeyword, DeleteKeyword } from './utilities/keywordactions.js';

export interface KeywordCollectionModifier extends KeywordCollection {
    keywordGuid: string;
    items: KeywordCollectionItem[];
    addKeyword(keyword: KeywordItem): void;
    updateKeyword(oldKeywordValue: string, newKeyword: KeywordItem): void;
    deleteKeyword(typeId: string, specificValue?: string): void;
    deleteKeywordGroup(typeGroupId: string, instanceId?: string): void;
    addKeywordGroup(groupMod: KeywordGroupModifier): void;
}
export class KeywordModifier implements KeywordCollectionModifier {
    keywordGuid: string;
    items: KeywordCollectionItem[];
    ...
    async addKeyword(keyword: KeywordItem): Promise<void> {
        const existingKeywords = await Promise.all(this.items.filter(async (item) => {
            if (keywordInItem(item, keyword.typeId)){
                if (!item.typeGroupId && !item.instanceId) {
                    await AddKeyword(item, keyword);
                }
                else if (item.typeGroupId && !item.instanceId) {
        ...
        if (!existingKeywords) {
            throw new Error(`Keyword Collection does not contain any keywords with the typeId ${keyword.typeId}.`);
        }
    }
    /**
    ...
    async updateKeyword(oldKeywordValue: string, newKeyword: KeywordItem): Promise<void> {
        ...
        this.items.forEach(async (item) => {
            if (keywordInItem(item, newKeyword.typeId)) {
                if (!item.typeGroupId && !item.instanceId) {
...
export class DocumentKeywordModifier extends KeywordModifier {
    declare keywordGuid: string;
    declare items: KeywordCollectionItem[];
    documentId?: string;

    constructor(keywordCollection: KeywordRecordCollection, documentId?: string) {
        super(keywordCollection.keywordGuid, keywordCollection.items);
